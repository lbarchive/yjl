...
class FriendFeed(object):
    ...
    def fetch_updates(self, **kwargs):
        ...
        return self._fetch_feed("/api/updates", server="chan.friendfeed.com", **kwargs)

    def fetch_updates_home(self, **kwargs):

        return self._fetch_feed("/api/updates/home", server="chan.friendfeed.com", **kwargs)
    ...
    def _fetch(self, uri, post_args, server="friendfeed.com", **url_args):
        url_args["format"] = "json"
        args = urllib.urlencode(url_args)
        url = "http://%s" % server + uri + "?" + args
